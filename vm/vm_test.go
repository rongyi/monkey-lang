...
func testStringObject(expected string, actual object.Object) error {
	...
	if ret.Value != expected {
		return fmt.Errorf("object has wrong value. got=%q, want=%q", ret.Value, expected)
	}
...
func TestCallingFunctionsWithBinds(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
...
func TestCallingFunctionWithArgs(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
let identity = fn(a) {a;};
...
func TestCallingFunctionsWithoutArgumentsAndBindings(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
let sum = fn(a, b) {
...
`,
	...
	for _, tt := range tests {
		...
		err = vm.Run()
		if err == nil {
			t.Fatalf("expected VM error but resulted in non.")
		}
